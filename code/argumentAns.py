...
cop_pos = 17
max_cases = 508
one_cases_es = [7, 38, 41, 43, 53, 55, 59, 60, 64, 65, 70, 89, 98, 116, 129, 136, 153, 154, 162, 169, 173, 176, 203, 208, 209, 212, 213, 214, 229, 236, 237, 238, 245, 252, 254, 262, 264, 277, 282, 292, 295, 298, 299, 310, 320, 326, 333, 341, 344, 352, 372, 385, 388, 448, 458, 465, 470, 482, 488, 504]
...
part_ans_cases_es = [13, 14, 15, 17, 19, 22, 23, 24, 25, 27, 31, 47, 48, 52, 54, 58, 63, 66, 67, 71, 77, 78, 84, 85, 88, 90, 92, 94, 96, 99, 100, 101, 102, 105, 107, 108, 114, 115, 118, 119, 121, 122, 123, 124, 126, 127, 128, 130, 131, 132, 133, 140, 142, 146, 147, 149, 152, 159, 160, 167, 174, 175, 177, 178, 179, 181, 184, 194, 196, 200, 206, 207, 210, 215, 216, 218, 219, 222, 223, 226, 230, 231, 233, 235, 239, 247, 265, 271, 273, 281, 284, 288, 322, 331, 374, 400, 415, 437, 493,8, 10, 35, 37, 44, 69, 97, 117, 165, 188, 189, 190, 195, 197, 205, 243, 246, 255, 256, 258, 263, 267, 268, 269, 270, 272, 276, 278, 279, 280, 289, 294, 306, 307, 311, 313, 317, 319, 321, 324, 330, 332, 336, 338, 346, 348, 349, 351, 357, 360, 375, 377, 378, 379, 380, 383, 386, 387, 390, 391, 392, 393, 394, 396, 399, 401, 407, 412, 418, 421, 422, 423, 425, 426, 435, 441, 442, 444, 445, 446, 453, 460, 466, 471, 472, 473, 474, 475, 477, 478, 479, 480, 483, 484, 487, 489, 492, 494, 495, 500, 501, 507,251, 253, 275, 283, 287, 308, 315, 318, 328, 337, 339, 340, 342, 364, 369, 397, 406, 409, 411, 430, 432, 440, 449, 450, 451, 456, 468, 486, 498]
all_ans_cases_es = [13, 14, 15, 17, 19, 22, 23, 24, 25, 27, 31, 47, 48, 52, 54, 58, 63, 66, 67, 71, 77, 78, 84, 85, 88, 90, 92, 94, 96, 99, 100, 101, 102, 105, 107, 108, 114, 115, 118, 119, 121, 122, 123, 124, 126, 127, 128, 130, 131, 132, 133, 140, 142, 146, 147, 149, 152, 159, 160, 167, 174, 175, 177, 178, 179, 181, 184, 194, 196, 200, 206, 207, 210, 215, 216, 218, 219, 222, 223, 226, 230, 231, 233, 235, 239, 247, 265, 271, 273, 281, 284, 288, 322, 331, 374, 400, 415, 437, 493, 255, 256, 258, 263, 267, 268, 269, 270, 272, 276, 278, 279, 280, 289, 294, 306, 307, 311, 313, 317, 319, 321, 324, 330, 332, 336, 338, 346, 348, 349, 351, 357, 360, 375, 377, 378, 379, 380, 383, 386, 387, 390, 391, 392, 393, 394, 396, 399, 401, 407, 412, 418, 421, 422, 423, 425, 426, 435, 441, 442, 444, 445, 446, 453, 460, 466, 471, 472, 473, 474, 475, 477, 478, 479, 480, 483, 484, 487, 489, 492, 494, 495, 500, 501, 507]
...
file_en = "results_en/"
file_es = "results_es/"
test_en = "test_results_en.csv"
test_es = "test_results_es.csv"
...
def get_at_position(path, num_case, position):
    cont =  0
    ...
    for line in mycsv:
        if first:
            ...
            if cont == num_case:
                return [s.lower() for s in line[position].split(",")]
            cont += 1
    return None
...
def in_common_with_levehstein(signs, lista, lev):
    count = 0
    for s in signs:
        for cs in lista:
            dist = levenshtein(s.lower(), cs)
            if dist <= lev:
                count += 1
    return count

def argument_correct_ans(file, number, quest_sign_path, lev):
    check_signs = get_at_position(quest_sign_path, number, 3)
    check_diseases = get_at_position(quest_sign_path, number, 1)
    ...
    for line in mycsv:
        if first:
            ...
            signs = line[pos_sign]
            if lev is None:
                common_num = in_common(signs, check_signs)  + in_common(signs, check_diseases)
            else:
                common_num = in_common(signs, check_signs) + in_common(signs, check_diseases) + in_common_with_levehstein(signs, check_signs, lev)
                #+ in_common_with_levehstein(signs, check_diseases, lev)
            check_num[line[0]] = common_num
    ...
    return correct_ans #possible_ans

def write_correct_ans(write_ans_path, correct_ans, quest_num):
    myFile = open(write_ans_path, 'a')
    writer = csv.writer(myFile)
    line = [quest_num, correct_ans]
   # line = [quest_num, ",".join(correct_ans)]
    writer.writerow(line)

def get_diseases_signs_and_argument(file, quest_sign_path, write_ans_path, lev):
    for i in range(max_cases):
        if (i not in not_cases_es and file.find("es") != -1) or (i not in not_cases_en and file.find("en") != -1) :
            ...
            first = True
            pos_sign, pos_disease, signs, diseases = [], [], [], []
            rows = []
            cont = 0
        ...
        correct_ans = argument_correct_ans("./" + file + str(i) + '_merged.csv', i, quest_sign_path, lev)
        write_correct_ans(write_ans_path, correct_ans, i)

def merge(input_path, paths, file_merge):
    for i in range(max_cases):
        df = pd.DataFrame()
        all_paths = []
        for path in paths:
...
def check(test_path, check_path, language):
    pos_test_opc = 1
    ...
    for line in mycsv:
        if first:
            ...
            cop = get_correct_option(check_path, count)
            #print(cop)
            #print(line[pos_test_opc])
            if cop is None:
                print("ERROR: it's not exit " + str(count) +" case")
            elif cop == line[pos_test_opc]:
    ...
    print("\tErantzun kopuru zuzenak: " + str(correct_ans_all2) + ", erantzun kopuru totala: " + str(len(eval(type2+language))))
    print("\tThe accuracy: " + str(correct_ans_all2 / len(eval(type2+language))))
    print(" ACCURACY WITH ALL CASES WITH ALL ANSWER FULL OR ALL CASES WITH 4 ANSWER FULL: ")
    print("\tErantzun kopuru zuzenak: " + str(correct_ans_all1) + ", erantzun kopuru totala: " + str(len(eval(type1 + language))))
    print("\tThe accuracy: " + str(correct_ans_all1 / len(eval(type1 + language))))

def merge_and_getSigns(language, input_path, quest_sign_path, check_path, lev):
    createFile(eval("file_"+language))
    first_line = ['QuestZbki', 'cop']
    createFile_and_write(eval("test_"+language), first_line)
    ...
    merge(input_path, paths, eval("file_"+language))
    get_diseases_signs_and_argument(eval("file_"+language), quest_sign_path, eval("test_"+language), lev)
    check(eval("test_"+language), check_path, language)

def main(input_path: str, quest_sign_path:str, check_path:str, language:str, lev:int):
    if os.path.isdir(input_path) and os.path.isfile(quest_sign_path) and os.path.isfile(check_path):
        ...
        if language != "es" and language != "en":
            ...
            merge_and_getSigns(language, input_path, quest_sign_path, check_path, lev)
    else:
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 5 and len(sys.argv) != 7:
        print("Usage: {} (--l THRESHOLD) input_source quest_sign_source check_source language".format(sys.argv[0]))
    else:
        ...
        main(args.arg1, args.arg2, args.arg3, args.lan, args.l)
