...
def write_csv(ans_number, names, names_codes, no_names, no_names_codes, output_path):
    ...
    unique_elements = list(unique(no_names))

    for elem in unique_elements:

        indices = [i for i, x in enumerate(no_names) if x[0] == elem]
        grouped_b = [no_names_codes[i][0] for i in indices]
        result_a.append(elem)
        result_b.append(grouped_b)
    ...
    str_no_names += ", ".join(result_a)

    for i in range(len(result_b)):
        str_no_codes += "# ".join(result_b[i]) + ", "
